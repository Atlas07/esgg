  ...
  let unrecord t name ty =
    let rec map name ty =
      ...
      | List (loc, ty, annot) ->
        let name =
          match name with
          | Some name when String.ends_with name ~suffix:"s" && String.length name > 1 -> Some (String.slice ~last:(-1) name)
...
*)
  ...
  let rec map shape =
    ...
    | Dict ["total is int or object", Simple Int] -> tname "total_hits"
